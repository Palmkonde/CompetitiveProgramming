...
struct QuickSumDiamon
{
    ...
    QuickSumDiamon(int r, int c, const Vec<Vec<int>> &inp) : input_row(r),
                                                             input_col(c),
...
public:
    ...
    */
    int area_diamon(int i, int j, int distance) const
    {
        ...
        return __area_inverse_piramit(i_outer, j_outer) - __area_inverse_piramit(i_outer, j_inner) - __area_inverse_piramit(i_inner, j_outer) + __area_inverse_piramit(i_inner, j_inner);
    }
